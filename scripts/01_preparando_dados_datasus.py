...
dataset = pd.read_csv("../data/sinan_datasus/sinannet_cnv_animaisbr.csv", 
...
cols_old = dataset.columns.to_list()
# lista de novos nomes de colunas
#cols_new = ['nnae_2009','nnae_2019']
cols_new = ['municipio', 'estado', 'nnae_2009','nnae_2019']
...
df = dataset.rename(columns=col_mapping).copy()

df.sort_values(by='municipio').reset_index(drop=True, inplace=True)
df.dtypes
...
dataset.to_csv("../data/sinan_datasus/dataset_nnae.csv", encoding='utf-8', index=False)